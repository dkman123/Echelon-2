...
$postTrack = implode_r("", filter_input_array(INPUT_POST));

$cracktrack = filter_input(INPUT_SERVER, 'QUERY_STRING');
...
{
    $cremotead = filter_input(INPUT_SERVER, 'REMOTE_ADDR');
    $cuseragent = filter_input(INPUT_SERVER, 'HTTP_USER_AGENT');
...
Translated: ".filter_input(INPUT_SERVER, 'PATH_TRANSLATED')."
Referrer: ".filter_input(INPUT_SERVER, 'HTTP_REFERRER')."
";
    
    //mail(EMAIL, "CTracker: Attack detected!", $mail, "From: ctracker@".htmlentities(filter_input(INPUT_SERVER, 'HTTP_HOST')));
    sendCTrackerMail("CTracker: Attack detected!", $mail);
...
function sendCTrackerMail($subject, $body){

    $mgr = new messenger(); 
    $mgr->to($email_config['admin_email']);
