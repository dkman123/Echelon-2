...
if($type_admin) {
    ...
    $query = "SELECT p.type, p.time_add, p.time_expire, p.reason, p.data, p.duration, p.client_id, c.name as client_name, c.ip as client_ip, '' AS admins_id, '' AS admins_name FROM penalties p LEFT JOIN clients c ON p.client_id = c.id WHERE p.admin_id = 0 AND p.type != 'Warning' AND p.inactive = 0";
}
