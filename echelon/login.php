...
if($mem->loggedIn()) { ## if logged in users may skip this page
...
} elseif (filter_input(INPUT_POST, 'f-name')) { ## if this is a log in request 
    ...
    // set sent vars
    $username = cleanvar(filter_input(INPUT_POST, 'f-name')); // strip and remove spaces from vars
    $pw = filter_input(INPUT_POST, 'f-pw');
    $game_input = cleanvar(filter_input(INPUT_POST, 'f-game'));
...
} elseif(filter_input(INPUT_POST, 'lostpw')) { // if this is a lost password first step submit
    ...
    $body .= 'This email is about how to reset your password on Echelon. Please do not foward this message on to anyone, this is a private email.
        ...
        <a href="http://'.filter_input(INPUT_SERVER, 'SERVER_NAME').$path.'login.php?t=reset&amp;key='.$key.'&amp;email='.$email.'"> >>>>>>>>Reset your password<<<<<<<<< </a>.<br />';
...
} elseif(filter_input(INPUT_POST, 'resetpw')) {
    ...
    // get and clean vars
    $pw1 = filter_input(INPUT_POST, 'pw2');
    $pw2 = filter_input(INPUT_POST, 'pw1');
...
} elseif(filter_input(INPUT_GET, 't') == 'reset') { // if this is a reset password
    ...
    $key = cleanvar(filter_input(INPUT_GET, 'key'));
    $email = cleanvar(filter_input(INPUT_GET, 'email'));
...
} elseif(filter_input(INPUT_GET, 't') == 'lost') { // if this is a lost password page
