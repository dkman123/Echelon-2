...
$query = "SELECT p.time_add, p.reason, target.id as target_id, target.name as target_name, target.ip as target_ip, c.id as admin_id, c.name as admins_name, p.time_expire, p.duration FROM penalties p, clients c, clients as target WHERE admin_id != '0' AND p.type = 'Kick' AND inactive = 0 AND p.client_id = target.id AND p.admin_id = c.id";
