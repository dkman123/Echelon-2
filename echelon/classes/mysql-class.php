...
if (!empty(filter_input(INPUT_SERVER, 'SCRIPT_FILENAME')) && 'mysql-class.php' == basename(filter_input(INPUT_SERVER, 'SCRIPT_FILENAME'))) {
...
class DB_B3 {
    ...
    public function query($sql, $fetch = true, $type = 'select') {
        ...
        if(empty($sql) || $this->mysql == NULL || $this->error) {
            $results['num_rows'] = 0;
            return $results;
        }
    ...
    function getB3Groups() {
        $query = "SELECT id, name FROM usergroups ORDER BY id ASC";
    ...
    function getB3GroupsLevel() {
        $query = "SELECT id FROM usergroups ORDER BY id ASC";
        $stmt = $this->mysql->prepare($query);
        $stmt->execute();
        $stmt->bind_result($id);
    ...
    */
     ...
     * Deletes a mapconfig
     *
     * @param string $map_id - id of mapconfig to deactivate
     * @return bool
     */
