...
if(filter_input(INPUT_GET, 't') == 'add') : // add group perms

    # verify token #
    //if(!verifyFormToken('perm-group-add', $tokens))
    //	ifTokenBad('Add group');

    $name = cleanvar(filter_input(INPUT_POST, 'g-name'));

    $data = getPostsPerms(filter_input_array(INPUT_POST));
    ...
    $group_id = cleanvar(filter_input(INPUT_GET, 'gid')); // get the group to update from the URL
