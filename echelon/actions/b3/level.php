...
if(filter_input(INPUT_POST, 'level-sub')) { // what kind of request is it
    $is_mask = false;
}
else {
    $is_mask = true;
...
## Set and clean vars ##
$level = cleanvar(filter_input(INPUT_POST, 'level'));
$client_id = cleanvar(filter_input(INPUT_POST, 'cid'));
$old_level = cleanvar(filter_input(INPUT_POST, 'old-level'));
$password = cleanvar(filter_input(INPUT_POST, 'password'));
