...
// set vars
$display = cleanvar(filter_input(INPUT_POST, 'name'));
$email = cleanvar(filter_input(INPUT_POST, 'email'));
$cur_pw = cleanvar(filter_input(INPUT_POST, 'password'));
$change_pw = filter_input(INPUT_POST, 'change-pw'); // password is being hashed no need to validate
$timezone = cleanvar(filter_input(INPUT_POST, 'timezone')); 

if(!filter_input(INPUT_POST, 'pass1') == '') { // check to see if the password is to be changed
