...
if(filter_input(INPUT_GET, 't')) {
    $t_param = filter_input(INPUT_GET, 't');
}
else {
    $t_param = "";
}
...
if (!empty($t_param)) {
    if($t_param == 'perms' OR $t_param == 'perms-group' OR $t_param == 'perms-add') {
            $auth_name = 'edit_perms';
    }
    elseif($t_param == 'user') {
            $auth_name = 'siteadmin';
    }
    elseif($t_param == 'edituser') {
            $auth_name = 'edit_user';
    }		
} 
else {
...
$is_edit_user = false;
if($t_param == 'edituser') :
    if(!isID(filter_input(INPUT_GET, 'id'))) {
        set_error('Invalid data sent. Request aborted.');
        send('sa.php');
    } else {
...
if($t_param == 'perms-group') :
	
    $group_id = cleanvar(filter_input(INPUT_GET, 'id'));
...
<span class="float-right"><span class="float-right"><?php echo delUserLink($id, $token_del)?></span><?php echo editUserLink($id, $username); ?></span></h5>
...
<div class="card-body">	
    ...
    <?php
        ...
        $rows = ceil($perms_count / 5) + 1; // the number of rows we'll need to display the data using X columns per row
        $ir = 1; // in row
        $idx = 0; // index (the data row from the table)
        ...
        while($ir < $rows) :
            ...
            while($col <= 5 && $idx < count($perms)) :
				
                $p_id = $perms[$idx]['id'];
                $p_name = $perms[$idx]['name'];
                $p_desc = $perms[$idx]['desc'];
...
<div class="card-body">	
    ...
    <?php
        ...
        $perms_count = count($perms);
        $rows = ceil($perms_count / 5) + 1;  // number of rows needed
        $ir = 1; // row
        $idx = 0;  // index
        ...
        <?php
            ...
            if($num_rows > 0) :
                ...
                foreach($keys_data['data'] as $reg_keys): // get data from query and loop
                    ...
                    if($mem->id == $admin_id) { // if the current user is the person who created the key allow the user to edit the key's comment
